...
(() => {
  ...
  if (!voidSite) return; // Not on the landing page
  ...
  function setMenuOpen(isOpen) {
    if (!landingMenu || !menuTrigger || !menuOverlay) return;
    if (isOpen) updateMenuAnchor();
  ...
  function animate() {
    ...
    if (landingMenu && topMark && featuresSection && entrySection) {
      ...
      if (menuOpacity < 0.08) setMenuOpen(false);
    }

    if (landingMenu?.classList.contains("is-open")) updateMenuAnchor();
